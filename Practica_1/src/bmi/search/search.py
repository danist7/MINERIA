...
# Clase que representa la busqueda por producto escalar
...
class VSMDotProductSearcher(Searcher):
    ...
    def search(self, query, cutoff):
        ...
        for term in terms:
            ...
            idf = math.log((self.index.ndocs() + 1) /
                           (self.index.doc_freq(term) + 0.5))
...
# Clase que representa la busqueda por el angulo de coseno


class VSMCosineSearcher(VSMDotProductSearcher):

    def search(self, query, cutoff):
        # Obtenemos los productos escalares usando la clase padre
        prodvec = super().search(query, -1)
        # Obtenemos los modulos de cada documento
        f = open("./index/modulos.txt", "r")
        # Realizamos el calculo para cada documento
        for id in prodvec:
